...
public class Currency1_19 implements Currency {
            ...
            if (!CurrenciesConfig.get().contains(name)) {
                ...
                if (name.length() > 2 && name.length() <= 9) {
                    ...
                    if (isNameValid) {
                        ...
                        // updating the player config
                        if (!PlayersConfig.get().contains(id + ".DET")) {
                            PlayersConfig.get().set(id + ".DET", DF.format(date));
                            CurrenciesAPI.PlayerAccounts.add(new PlayerAccount(id, DF.format(date), pwallet));
                        }
                        int index = IntStream.range(0, CurrenciesAPI.PlayerAccounts.size())
                                .filter(i -> CurrenciesAPI.PlayerAccounts.get(i).player_name.equals(id))
                                .findFirst()
                                .orElse(-1);
                        CurrenciesAPI.PlayerAccounts.get(index).wallet = pwallet;
    ...
    public boolean help(CommandSender s, int pagenum){
        s.sendMessage(ChatColor.GOLD + LanguageConfig.get().getString(LanguageConfig.get().getString("language") + ".message-7_1") + pagenum + " | \n \n");
        StringBuilder helptext = new StringBuilder();
        helptext.append(ChatColor.GREEN + LanguageConfig.get().getString(LanguageConfig.get().getString("language") + ".message-7"));
        ChatPaginator.ChatPage page = paginate(helptext.toString(), pagenum, 200, 5);
    ...
    public boolean payOffline(Player p, OfflinePlayer target, String name, double amount) {
        ...
        if(CurrenciesAPI.playersConfig.get().contains(targetidd)) {
            //if currency exists
            if (CurrenciesConfig.get().contains(name)) {
                //if you're trying to pay yourself
                if (target == p) {
            ...
            p.sendMessage(ChatColor.DARK_RED + LanguageConfig.get().getString(LanguageConfig.get().getString("language") + ".error-9_5"));
        }
        return true;
    }
    ...
    public boolean top(String name, CommandSender p, int pagenum) {
        ...
        if(CurrenciesAPI.currenciesConfig.get().contains(name)) {
            p.sendMessage(ChatColor.GOLD + LanguageConfig.get().getString(LanguageConfig.get().getString("language") + ".message-13") + " " + name + " | " + LanguageConfig.get().getString(LanguageConfig.get().getString("language") + ".message-13_1") + pagenum + " | \n \n");
            PlayersConfig.reload();
            for (String player : PlayersConfig.get().getKeys(false)) {
            ...
            scoreboard.sort(myComparator);
            Collections.reverse(scoreboard);
            int i = 0;
            for (Tuple<String, Double> playerBalance : scoreboard) {
                i++;
                leaderboardText.append(ChatColor.RED + "     " + i + ". " + playerBalance.First + ": " + ChatColor.GREEN + playerBalance.Second + "\n ");
            }
            ChatPaginator.ChatPage page = paginate(String.valueOf(leaderboardText), pagenum, 40, 5);
            for (String line : page.getLines()) {
                p.sendMessage(line);
            }
        } else {
    ...
    public static ChatPaginator.ChatPage paginate(String unpaginatedString, int pageNumber, int lineLength, int pageHeight){
        return ChatPaginator.paginate(unpaginatedString, pageNumber, lineLength, pageHeight);
    }
